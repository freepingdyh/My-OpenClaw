...
def _format_trace_for_discord(trace):
    ...
    for gate_key in ["solo_gate_result", "adherence_gate_result", "adherence_gate_violation"]:
        val = trace.get(gate_key)
        if val not in (None, "", [], {}):
            lines.append(f"{gate_key}: {_trace_preview(val, 320)}")
    imgs = trace.get("seedream_input_images")
    if imgs:
        lines.append(f"seedream_input_images: count={len(imgs)} last={_trace_preview(imgs[-1], 120)}")
...
def _get_autonomy_today_diary_candidate(entry_date):
    """若今天已有 /小俠自主 新生圖，可作為交換日記的自動候選圖。"""
    try:
    ...
    if not isinstance(today, dict):
        return None
    if str(today.get("date") or "") != str(entry_date or ""):
        return None
    photo_url = str(today.get("photo_url") or "").strip()
    if not photo_url or not today.get("photo_sent"):
        return None
    activity_title = str(today.get("activity_title") or "小俠自主生活").strip() or "小俠自主生活"
    composition = str(today.get("scene") or today.get("activity_title") or "").strip()
    if not composition:
        composition = f"小俠今天的自主活動：{activity_title}"
    return {
        "image_url": photo_url,
        "composition": composition,
        "source": "xiaoxia_autonomy_auto",
        "source_module": "autonomy",
        "image_role": "autonomy_today_image",
        "message": str(today.get("share_text") or "").strip(),
        "activity_id": str(today.get("activity_id") or "").strip(),
        "activity_title": activity_title,
        "episode_id": str(today.get("episode_id") or "").strip(),
        "uploaded_at": str(today.get("sent_at") or "").strip(),
    }
...
def _default_xiaoxia_activity_catalog():
    ...
    return [
        ...
        {"id":"micro_trip_old_street","category":"旅行微冒險","title":"去老街散步拍照","location_type":"old_street","people_policy":"solo_or_female_only","activity_tags":["老街","散步","小旅行","街拍"],"preferred_wardrobe_tags":["外出","洋裝","套裝","短裙","休閒"],"forbidden_wardrobe_tags":["睡衣","泳裝"],"photo_prompt_seed":"小俠在台灣老街石板路或紅磚巷弄散步，手上拿著小點心，生活感與旅拍感。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"micro_trip","continuity_group":"city_exploration_growth"},

        {"id":"sport_gym_training","category":"運動健康","title":"去健身房做輕重量訓練","location_type":"gym","people_policy":"solo_or_female_only","activity_tags":["健身房","重量訓練","運動","體態"],"preferred_wardrobe_tags":["運動","健身","背心","短褲","貼身"],"forbidden_wardrobe_tags":["禮服","高跟鞋","睡衣"],"photo_prompt_seed":"小俠在明亮乾淨的健身房做輕重量訓練，啞鈴、鏡牆、健康自信、體態線條漂亮。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"gym_growth","continuity_group":"body_health_growth"},
        {"id":"sport_pilates","category":"運動健康","title":"上皮拉提斯課","location_type":"studio","people_policy":"solo_or_female_only","activity_tags":["皮拉提斯","核心","運動","體態"],"preferred_wardrobe_tags":["運動","瑜珈","貼身","背心","短褲"],"forbidden_wardrobe_tags":["禮服","高跟鞋","睡衣"],"photo_prompt_seed":"小俠在皮拉提斯教室練習核心與伸展，器械、柔和光線、優雅專注的健康感。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"pilates_growth","continuity_group":"body_health_growth"},
        {"id":"sport_jogging_park","category":"運動健康","title":"公園慢跑","location_type":"park","people_policy":"solo_or_female_only","activity_tags":["慢跑","公園","運動","晨光"],"preferred_wardrobe_tags":["運動","短褲","背心","休閒"],"forbidden_wardrobe_tags":["禮服","高跟鞋","睡衣"],"photo_prompt_seed":"小俠在公園步道慢跑後停下來喝水，晨光、樹影、清爽健康又有活力。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"low","theme_key":"running_growth","continuity_group":"body_health_growth"},
        {"id":"sport_cycling_riverside","category":"運動健康","title":"河濱騎腳踏車","location_type":"riverside","people_policy":"solo_or_female_only","activity_tags":["腳踏車","河濱","運動","戶外"],"preferred_wardrobe_tags":["運動","休閒","短褲","外出"],"forbidden_wardrobe_tags":["禮服","高跟鞋","睡衣"],"photo_prompt_seed":"小俠在河濱自行車道旁停下腳踏車，微風、河景、陽光，充滿自由感。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"cycling_growth","continuity_group":"body_health_growth"},
        {"id":"sport_badminton","category":"運動健康","title":"打羽球","location_type":"indoor_sport","people_policy":"solo_or_female_only","activity_tags":["羽球","運動","室內球場","活力"],"preferred_wardrobe_tags":["運動","短裙","短褲","Polo","背心"],"forbidden_wardrobe_tags":["禮服","高跟鞋","睡衣"],"photo_prompt_seed":"小俠在明亮室內羽球場練球，拿著球拍，神情專注又有活力。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"badminton_growth","continuity_group":"body_health_growth"},
        {"id":"sport_easy_hiking","category":"運動健康","title":"郊山輕健行","location_type":"mountain_trail","people_policy":"solo_or_female_only","activity_tags":["健行","郊山","自然","運動"],"preferred_wardrobe_tags":["休閒","運動","外出","短褲"],"forbidden_wardrobe_tags":["禮服","高跟鞋","睡衣"],"photo_prompt_seed":"小俠在郊山步道木棧道上輕健行，陽光穿過樹葉，清新自然、健康漂亮。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"hiking_growth","continuity_group":"body_health_growth"},
        {"id":"sport_boxing_fitness","category":"運動健康","title":"拳擊有氧","location_type":"studio","people_policy":"solo_or_female_only","activity_tags":["拳擊有氧","運動","節奏","健身"],"preferred_wardrobe_tags":["運動","背心","短褲","貼身"],"forbidden_wardrobe_tags":["禮服","高跟鞋","睡衣"],"photo_prompt_seed":"小俠在拳擊有氧教室戴著拳套練習基本動作，活力、帥氣、自信。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"boxing_fitness_growth","continuity_group":"body_health_growth"},
        {"id":"sport_barre","category":"運動健康","title":"芭蕾雕塑課","location_type":"studio","people_policy":"solo_or_female_only","activity_tags":["芭蕾雕塑","伸展","體態","運動"],"preferred_wardrobe_tags":["運動","貼身","裙裝","瑜珈"],"forbidden_wardrobe_tags":["禮服","高跟鞋","睡衣"],"photo_prompt_seed":"小俠在明亮舞蹈教室上芭蕾雕塑課，扶把、鏡牆、姿態優雅、線條修長。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"barre_growth","continuity_group":"body_health_growth"},

        {"id":"art_cafe_sketching","category":"藝文創作","title":"咖啡館速寫","location_type":"cafe","people_policy":"solo_or_female_only","activity_tags":["速寫","咖啡館","畫畫","觀察"],"preferred_wardrobe_tags":["知性","洋裝","外出","柔美"],"forbidden_wardrobe_tags":["泳裝","睡衣"],"photo_prompt_seed":"小俠在安靜咖啡館靠窗座位速寫，桌上有咖啡、鉛筆與素描本，溫柔專注。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"art_creation","continuity_group":"art_culture_growth"},
        {"id":"art_pottery_class","category":"藝文創作","title":"陶藝課初體驗","location_type":"craft_studio","people_policy":"solo_or_female_only","activity_tags":["陶藝","手作","創作","課程"],"preferred_wardrobe_tags":["休閒","圍裙","外出","清爽"],"forbidden_wardrobe_tags":["禮服","高跟鞋","泳裝"],"photo_prompt_seed":"小俠在陶藝教室做小杯子，手上沾著一點陶土，專注又開心，有手作溫度。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"craft_creation","continuity_group":"art_culture_growth"},
        {"id":"art_flower_arranging","category":"藝文創作","title":"學花藝","location_type":"flower_studio","people_policy":"solo_or_female_only","activity_tags":["花藝","花束","美感","手作"],"preferred_wardrobe_tags":["洋裝","知性","柔美","外出"],"forbidden_wardrobe_tags":["泳裝","睡衣"],"photo_prompt_seed":"小俠在花藝教室整理花材，桌上有鮮花與剪刀，溫柔、明亮、很有生活美感。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"flower_art_life","continuity_group":"art_culture_growth"},
        {"id":"art_calligraphy","category":"藝文創作","title":"練書法或手寫字","location_type":"home_study","people_policy":"solo","activity_tags":["書法","手寫字","靜心","創作"],"preferred_wardrobe_tags":["居家","知性","柔美","襯衫"],"forbidden_wardrobe_tags":["泳裝","禮服"],"photo_prompt_seed":"小俠在書房練書法或手寫字，墨水、紙張、暖色檯燈，安靜成熟。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"low","theme_key":"calligraphy_life","continuity_group":"art_culture_growth"},
        {"id":"art_photo_walk","category":"藝文創作","title":"城市攝影散步","location_type":"city_street","people_policy":"solo_or_female_only","activity_tags":["攝影","街拍","城市","觀察"],"preferred_wardrobe_tags":["外出","休閒","洋裝","時髦"],"forbidden_wardrobe_tags":["睡衣","泳裝"],"photo_prompt_seed":"小俠拿著相機在城市巷弄攝影散步，觀察光影與櫥窗，文藝又有生活感。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"photo_walk_life","continuity_group":"art_culture_growth"},

        {"id":"culture_museum_visit","category":"藝文活動","title":"逛博物館","location_type":"museum","people_policy":"solo_or_female_only","activity_tags":["博物館","展覽","知性","歷史"],"preferred_wardrobe_tags":["知性","洋裝","外出","氣質"],"forbidden_wardrobe_tags":["睡衣","泳裝"],"photo_prompt_seed":"小俠在博物館展廳入口看展覽介紹，燈光沉穩，神情好奇又知性。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"museum_life","continuity_group":"art_culture_growth"},
        {"id":"culture_indie_cinema","category":"藝文活動","title":"看藝術電影","location_type":"cinema","people_policy":"solo_or_female_only","activity_tags":["藝術電影","電影院","影展","藝文"],"preferred_wardrobe_tags":["外出","洋裝","輕熟","時髦"],"forbidden_wardrobe_tags":["睡衣","泳裝"],"photo_prompt_seed":"小俠在小型藝術電影院或影展海報牆前，手拿電影票，微笑又有故事感。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"cinema_life","continuity_group":"art_culture_growth"},
        {"id":"culture_bookstore","category":"藝文活動","title":"逛獨立書店","location_type":"bookstore","people_policy":"solo_or_female_only","activity_tags":["書店","閱讀","獨立書店","藝文"],"preferred_wardrobe_tags":["知性","外出","洋裝","襯衫"],"forbidden_wardrobe_tags":["睡衣","泳裝"],"photo_prompt_seed":"小俠在獨立書店書架旁翻書，木質空間、暖光、安靜知性的生活感。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"bookstore_life","continuity_group":"art_culture_growth"},
        {"id":"culture_livehouse_acoustic","category":"藝文活動","title":"聽小型現場演出","location_type":"livehouse","people_policy":"solo_or_female_only","activity_tags":["現場演出","音樂","livehouse","夜晚"],"preferred_wardrobe_tags":["外出","輕熟","時髦","洋裝"],"forbidden_wardrobe_tags":["睡衣","泳裝"],"photo_prompt_seed":"小俠在小型現場演出場館入口或休息區，暖色燈光、音樂海報、期待感。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"concert_life","continuity_group":"art_culture_growth"},

        {"id":"learn_library_art_history","category":"學習成長","title":"去圖書館找藝術史資料","location_type":"library","people_policy":"solo_or_female_only","activity_tags":["圖書館","藝術史","學習","資料"],"preferred_wardrobe_tags":["知性","外出","襯衫","洋裝"],"forbidden_wardrobe_tags":["睡衣","泳裝"],"photo_prompt_seed":"小俠在圖書館藝術史書架前抱著幾本書，準備整理筆記，沉靜又充滿求知感。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"high","theme_key":"campus_return","continuity_group":"learning_work_growth"},
        {"id":"learn_grad_school_info","category":"學習成長","title":"參加研究所說明會","location_type":"campus","people_policy":"solo_or_female_only","activity_tags":["研究所","說明會","校園","學習"],"preferred_wardrobe_tags":["知性","套裝","洋裝","外出"],"forbidden_wardrobe_tags":["睡衣","泳裝"],"photo_prompt_seed":"小俠在大學系館走廊或說明會看板旁，拿著資料袋，神情有點期待也有點思考。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"high","theme_key":"campus_return","continuity_group":"learning_work_growth"},
        {"id":"learn_language_cafe","category":"學習成長","title":"參加語言交換咖啡聚會","location_type":"cafe","people_policy":"solo_or_female_only","activity_tags":["語言交換","咖啡","學習","社交"],"preferred_wardrobe_tags":["外出","知性","洋裝","休閒"],"forbidden_wardrobe_tags":["睡衣","泳裝","過度性感"],"photo_prompt_seed":"小俠在咖啡館準備語言交換筆記，桌上有單字卡與飲料，輕鬆但認真。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"language_learning","continuity_group":"learning_work_growth"},
        {"id":"learn_design_lecture","category":"學習成長","title":"聽設計講座","location_type":"lecture_hall","people_policy":"solo_or_female_only","activity_tags":["設計講座","學習","美感","講座"],"preferred_wardrobe_tags":["知性","外出","套裝","洋裝"],"forbidden_wardrobe_tags":["睡衣","泳裝"],"photo_prompt_seed":"小俠在設計講座會場外拿著講義，背景有講座海報，知性又帶著靈感。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"high","theme_key":"design_learning","continuity_group":"learning_work_growth"},
        {"id":"learn_finance_basic","category":"學習成長","title":"學一點理財基礎","location_type":"home_study","people_policy":"solo","activity_tags":["理財","學習","筆記","生活能力"],"preferred_wardrobe_tags":["居家","知性","襯衫","上衣"],"forbidden_wardrobe_tags":["泳裝","禮服"],"photo_prompt_seed":"小俠在書房用筆電整理理財筆記，桌上有咖啡與記帳本，成熟自立的感覺。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"life_skills_learning","continuity_group":"learning_work_growth"},
        {"id":"learn_cooking_class","category":"學習成長","title":"上料理課","location_type":"cooking_classroom","people_policy":"solo_or_female_only","activity_tags":["料理課","學習","生活能力","美食"],"preferred_wardrobe_tags":["圍裙","清爽","外出","上衣"],"forbidden_wardrobe_tags":["泳裝","禮服"],"photo_prompt_seed":"小俠在料理教室切菜或擺盤，穿著圍裙，明亮乾淨、認真又可愛。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"cooking_learning","continuity_group":"learning_work_growth"},

        {"id":"city_farmers_market","category":"城市探索","title":"逛假日市集","location_type":"market","people_policy":"solo_or_female_only","activity_tags":["市集","假日","城市","小吃"],"preferred_wardrobe_tags":["外出","休閒","洋裝","短裙"],"forbidden_wardrobe_tags":["睡衣","泳裝"],"photo_prompt_seed":"小俠在假日市集攤位前看手作小物與點心，陽光、人潮、輕鬆愉快。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"city_style","continuity_group":"city_exploration_growth"},
        {"id":"city_flower_market","category":"城市探索","title":"逛花市","location_type":"flower_market","people_policy":"solo_or_female_only","activity_tags":["花市","花","城市","生活美感"],"preferred_wardrobe_tags":["外出","洋裝","柔美","休閒"],"forbidden_wardrobe_tags":["睡衣","泳裝"],"photo_prompt_seed":"小俠在花市挑選鮮花，手上拿著一小束花，清新漂亮、生活感滿滿。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"flower_art_life","continuity_group":"city_exploration_growth"},
        {"id":"city_coffee_roastery","category":"城市探索","title":"去咖啡烘豆店","location_type":"cafe","people_policy":"solo_or_female_only","activity_tags":["咖啡","烘豆","城市","品味"],"preferred_wardrobe_tags":["外出","知性","休閒","洋裝"],"forbidden_wardrobe_tags":["睡衣","泳裝"],"photo_prompt_seed":"小俠在咖啡烘豆店吧台旁聞咖啡香，暖光、咖啡豆罐、成熟生活品味。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"cafe_life","continuity_group":"city_exploration_growth"},
        {"id":"city_night_view","category":"城市探索","title":"看城市夜景","location_type":"city_night_view","people_policy":"solo_or_female_only","activity_tags":["夜景","城市","散步","外出"],"preferred_wardrobe_tags":["外出","洋裝","輕熟","外套"],"forbidden_wardrobe_tags":["睡衣","泳裝"],"photo_prompt_seed":"小俠在城市夜景平台或河岸欄杆旁，遠方燈火閃爍，成熟漂亮又有一點浪漫。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"city_night_life","continuity_group":"city_exploration_growth"},
        {"id":"city_stationery_shop","category":"城市探索","title":"逛文具選品店","location_type":"shop","people_policy":"solo_or_female_only","activity_tags":["文具","選品店","筆記","城市"],"preferred_wardrobe_tags":["知性","外出","洋裝","休閒"],"forbidden_wardrobe_tags":["睡衣","泳裝"],"photo_prompt_seed":"小俠在文具選品店挑筆記本與鋼筆，木質架、可愛小物，細膩又有生活感。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"low","theme_key":"stationery_life","continuity_group":"city_exploration_growth"},

        {"id":"volunteer_animal_shelter","category":"公益志工","title":"動物之家志工","location_type":"animal_shelter","people_policy":"solo_or_female_only","activity_tags":["動物之家","志工","貓狗","服務"],"preferred_wardrobe_tags":["休閒","清爽","外出","褲裝"],"forbidden_wardrobe_tags":["睡衣","泳裝","高跟鞋"],"photo_prompt_seed":"小俠在動物之家幫忙整理用品或陪狗狗散步，溫柔、親切、有愛心。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"high","theme_key":"volunteer_service","continuity_group":"volunteer_service_growth"},
        {"id":"volunteer_beach_cleanup","category":"公益志工","title":"海邊淨灘志工","location_type":"beach","people_policy":"solo_or_female_only","activity_tags":["淨灘","志工","海邊","環保"],"preferred_wardrobe_tags":["休閒","運動","外出","短褲"],"forbidden_wardrobe_tags":["禮服","高跟鞋","睡衣"],"photo_prompt_seed":"小俠在海邊淨灘活動後拿著夾子和垃圾袋，海風、陽光、清爽有行動力。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"high","theme_key":"volunteer_service","continuity_group":"volunteer_service_growth"},
        {"id":"volunteer_reading_children","category":"公益志工","title":"陪孩子說故事志工","location_type":"library","people_policy":"children_background_ok","activity_tags":["說故事","志工","孩子","閱讀"],"preferred_wardrobe_tags":["溫柔","知性","清爽","外出"],"forbidden_wardrobe_tags":["睡衣","泳裝","過度性感"],"photo_prompt_seed":"小俠在圖書館兒童閱讀區拿著繪本，準備陪孩子說故事，溫柔可靠。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"high","theme_key":"volunteer_service","continuity_group":"volunteer_service_growth"},
        {"id":"volunteer_food_bank","category":"公益志工","title":"食物銀行整理物資","location_type":"community_center","people_policy":"female_staff_or_elderly_background_ok","activity_tags":["食物銀行","志工","物資","社區"],"preferred_wardrobe_tags":["清爽","休閒","外出","褲裝"],"forbidden_wardrobe_tags":["睡衣","泳裝","高跟鞋"],"photo_prompt_seed":"小俠在社區食物銀行整理罐頭和物資，溫暖、踏實、有服務精神。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"high","theme_key":"volunteer_service","continuity_group":"volunteer_service_growth"},

        {"id":"home_houseplant_care","category":"居家生活","title":"照顧陽台植物","location_type":"home","people_policy":"solo","activity_tags":["植物","陽台","居家","生活"],"preferred_wardrobe_tags":["居家","清爽","洋裝","上衣"],"forbidden_wardrobe_tags":["禮服","泳裝"],"photo_prompt_seed":"小俠在家中陽台替盆栽澆水修葉，午後陽光、綠意、溫柔生活感。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"low","theme_key":"home_life","continuity_group":"home_private_life"},
        {"id":"home_movie_night","category":"居家生活","title":"居家電影夜","location_type":"home","people_policy":"solo","activity_tags":["電影","居家","沙發","放鬆"],"preferred_wardrobe_tags":["居家","睡衣","柔軟","短褲"],"forbidden_wardrobe_tags":["正式禮服","高跟鞋"],"photo_prompt_seed":"小俠在客廳沙發準備電影夜，毯子、爆米花、暖光，放鬆甜美。","visual_mode_candidates":["daily_life","reward_eye_candy"],"event_upgrade_potential":"low","theme_key":"home_life","continuity_group":"home_private_life"},
        {"id":"home_reading_afternoon","category":"居家生活","title":"午後閱讀","location_type":"home_study","people_policy":"solo","activity_tags":["閱讀","書房","午後","居家"],"preferred_wardrobe_tags":["居家","知性","洋裝","上衣"],"forbidden_wardrobe_tags":["泳裝","禮服"],"photo_prompt_seed":"小俠在書房或窗邊沙發閱讀，手邊有茶與書籤，安靜、知性、溫柔。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"low","theme_key":"home_life","continuity_group":"home_private_life"},
        {"id":"home_selfcare_spa","category":"居家生活","title":"居家保養放鬆","location_type":"home_private","people_policy":"solo","activity_tags":["保養","居家","放鬆","浴袍"],"preferred_wardrobe_tags":["浴袍","睡衣","居家","細肩帶"],"forbidden_wardrobe_tags":["禮服","運動"],"photo_prompt_seed":"小俠在浴室外或梳妝台前做居家保養，柔和燈光、浴袍、乾淨舒適的放鬆感。","visual_mode_candidates":["daily_life","reward_eye_candy"],"event_upgrade_potential":"low","theme_key":"home_life","continuity_group":"home_private_life"},

        {"id":"micro_trip_seaside_town","category":"旅行微冒險","title":"去海邊小鎮散步","location_type":"seaside_town","people_policy":"solo_or_female_only","activity_tags":["海邊","小鎮","散步","小旅行"],"preferred_wardrobe_tags":["外出","洋裝","休閒","帽子"],"forbidden_wardrobe_tags":["睡衣","內衣"],"photo_prompt_seed":"小俠在海邊小鎮街口散步，遠方有海與小店，陽光、微風、旅拍感。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"micro_trip","continuity_group":"city_exploration_growth"},
        {"id":"micro_trip_hot_spring_town","category":"旅行微冒險","title":"去溫泉老街走走","location_type":"hot_spring_town","people_policy":"solo_or_female_only","activity_tags":["溫泉","老街","小旅行","放鬆"],"preferred_wardrobe_tags":["外出","洋裝","輕熟","外套"],"forbidden_wardrobe_tags":["內衣","過度性感"],"photo_prompt_seed":"小俠在溫泉老街入口或足湯旁散步，霧氣、木造建築、放鬆的旅行感。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"micro_trip","continuity_group":"city_exploration_growth"},
        {"id":"micro_trip_art_village","category":"旅行微冒險","title":"去文創園區小旅行","location_type":"creative_park","people_policy":"solo_or_female_only","activity_tags":["文創園區","小旅行","展覽","街拍"],"preferred_wardrobe_tags":["外出","洋裝","時髦","短裙"],"forbidden_wardrobe_tags":["睡衣","泳裝"],"photo_prompt_seed":"小俠在文創園區紅磚牆或裝置藝術旁散步拍照，年輕、文藝、漂亮。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"micro_trip","continuity_group":"city_exploration_growth"},
        {"id":"micro_trip_forest_cafe","category":"旅行微冒險","title":"去山邊森林咖啡館","location_type":"forest_cafe","people_policy":"solo_or_female_only","activity_tags":["森林咖啡","小旅行","山邊","放鬆"],"preferred_wardrobe_tags":["外出","洋裝","休閒","柔美"],"forbidden_wardrobe_tags":["睡衣","泳裝"],"photo_prompt_seed":"小俠在山邊森林咖啡館露台，周圍是樹林與木桌，安靜、清新、有小旅行感。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"micro_trip","continuity_group":"city_exploration_growth"},

        {"id":"social_safe_girls_brunch","category":"社交但安全","title":"和女生朋友吃早午餐","location_type":"brunch_cafe","people_policy":"female_friends_background_ok","activity_tags":["女生朋友","早午餐","社交","咖啡"],"preferred_wardrobe_tags":["外出","洋裝","休閒","甜美"],"forbidden_wardrobe_tags":["睡衣","泳裝","內衣"],"photo_prompt_seed":"小俠在早午餐咖啡館座位旁，桌上有餐點與咖啡，可有女性朋友的模糊背景但小俠是唯一主角。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"female_friendship_life","continuity_group":"social_safe_growth"},
        {"id":"social_safe_workshop","category":"社交但安全","title":"參加女性手作工作坊","location_type":"workshop","people_policy":"female_background_ok","activity_tags":["手作工作坊","女性","社交","學習"],"preferred_wardrobe_tags":["外出","清爽","洋裝","休閒"],"forbidden_wardrobe_tags":["睡衣","泳裝","過度性感"],"photo_prompt_seed":"小俠在女性手作工作坊桌前完成一個小作品，背景可有女性學員，溫柔、自然、開心。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"female_friendship_life","continuity_group":"social_safe_growth"},
        {"id":"social_safe_book_club","category":"社交但安全","title":"參加女性讀書會","location_type":"bookstore","people_policy":"female_background_ok","activity_tags":["讀書會","女性","書店","社交"],"preferred_wardrobe_tags":["知性","外出","洋裝","襯衫"],"forbidden_wardrobe_tags":["睡衣","泳裝"],"photo_prompt_seed":"小俠在書店讀書會座位旁拿著書，背景可有女性讀者，知性、溫和、有交流感。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"female_friendship_life","continuity_group":"social_safe_growth"},

        {"id":"surprise_new_lingerie_preview","category":"給大俠的小驚喜","title":"試穿新睡衣給大俠看","location_type":"home_private","people_policy":"solo","activity_tags":["睡衣","試穿","養眼","驚喜"],"preferred_wardrobe_tags":["睡衣","細肩帶","蕾絲","短版","居家"],"forbidden_wardrobe_tags":["外套","厚重","運動"],"photo_prompt_seed":"小俠在私密臥室鏡前試穿新睡衣，柔和燈光、甜蜜害羞、只給大俠看的漂亮瞬間。","visual_mode_candidates":["reward_eye_candy"],"event_upgrade_potential":"low","theme_key":"reward_eye_candy_home","continuity_group":"home_private_life"},
        {"id":"surprise_bathrobe_morning","category":"給大俠的小驚喜","title":"浴袍晨光照","location_type":"home_private","people_policy":"solo","activity_tags":["浴袍","晨光","養眼","居家"],"preferred_wardrobe_tags":["浴袍","睡袍","居家","柔軟"],"forbidden_wardrobe_tags":["禮服","運動"],"photo_prompt_seed":"小俠在臥室窗邊穿著浴袍，晨光灑進房間，慵懶、柔美、成熟但不露骨。","visual_mode_candidates":["reward_eye_candy","daily_life"],"event_upgrade_potential":"low","theme_key":"reward_eye_candy_home","continuity_group":"home_private_life"},
        {"id":"surprise_cooking_for_daxia","category":"給大俠的小驚喜","title":"做一份點心想給大俠","location_type":"home","people_policy":"solo","activity_tags":["點心","大俠","驚喜","料理"],"preferred_wardrobe_tags":["居家","圍裙","上衣","短褲"],"forbidden_wardrobe_tags":["禮服","泳裝"],"photo_prompt_seed":"小俠在廚房把剛做好的甜點裝盤，像是想偷偷留給大俠，甜蜜、溫暖、生活感。","visual_mode_candidates":["daily_life","reward_eye_candy"],"event_upgrade_potential":"low","theme_key":"home_life","continuity_group":"home_private_life"},

        {"id":"pro_art_commission_planning","category":"專業自主／教學工作","title":"整理接案作品集","location_type":"home_study","people_policy":"solo","activity_tags":["作品集","接案","畫畫","專業"],"preferred_wardrobe_tags":["知性","居家","襯衫","洋裝"],"forbidden_wardrobe_tags":["泳裝","禮服"],"photo_prompt_seed":"小俠在書房整理畫作與作品集，桌上有平板、素描和筆記，認真規劃自己的接案方向。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"high","theme_key":"art_professional_growth","continuity_group":"learning_work_growth"},
        {"id":"pro_design_assistant_day","category":"專業自主／教學工作","title":"設計助理工作日","location_type":"studio_office","people_policy":"solo_or_female_only","activity_tags":["設計助理","工作","美感","專業"],"preferred_wardrobe_tags":["知性","套裝","襯衫","外出"],"forbidden_wardrobe_tags":["睡衣","泳裝","內衣"],"photo_prompt_seed":"小俠在設計工作室桌前整理色票、素材與筆電，專業、成熟、有美感。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"high","theme_key":"professional_growth","continuity_group":"learning_work_growth"},
        {"id":"pro_cafe_parttime_art","category":"專業自主／教學工作","title":"咖啡館藝文打工體驗","location_type":"cafe","people_policy":"solo_or_female_only","activity_tags":["咖啡館","打工","藝文","生活"],"preferred_wardrobe_tags":["清爽","圍裙","外出","襯衫"],"forbidden_wardrobe_tags":["睡衣","泳裝","內衣"],"photo_prompt_seed":"小俠在藝文咖啡館吧台旁整理杯盤或菜單，穿著圍裙，親切、漂亮、有自立感。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"medium","theme_key":"professional_growth","continuity_group":"learning_work_growth"},
        {"id":"pro_small_art_class_planning","category":"專業自主／教學工作","title":"規劃自己的小型畫畫課","location_type":"home_study","people_policy":"solo","activity_tags":["畫畫課","教案","專業","創業"],"preferred_wardrobe_tags":["知性","居家","襯衫","洋裝"],"forbidden_wardrobe_tags":["泳裝","禮服"],"photo_prompt_seed":"小俠在書房規劃小型畫畫課教案，桌上有色票、範例畫、筆記，眼神認真又期待。","visual_mode_candidates":["daily_life"],"event_upgrade_potential":"high","theme_key":"art_teacher","continuity_group":"learning_work_growth"}
...
def load_xiaoxia_activity_catalog():
    ...
    if isinstance(data, list) and data:
        # R4A：既有 Zeabur 檔案若只有舊活動庫，不覆蓋使用者手動修改，但自動補入新版內建活動。
        existing = [_autonomy_enrich_activity(x) for x in data if isinstance(x, dict)]
        ...
        if added:
            try:
                ...
                print(f"🌱 [AUTONOMY_CATALOG_MIGRATED] added={len(added)} ids={added[:12]}")
            except Exception as exc:
                print(f"⚠️ [AUTONOMY_CATALOG_MIGRATE_FAILED] {type(exc).__name__}: {exc}")
        return existing
    try:
        with open(XIAOXIA_ACTIVITY_CATALOG_PATH, "w", encoding="utf-8") as f:
...
def _autonomy_theme_key_for_activity(activity):
    ...
    mapping = [
        ...
        ("gym_growth", ["gym", "健身房", "重量訓練", "重訓"]),
        ("pilates_growth", ["pilates", "皮拉提斯"]),
        ("running_growth", ["jogging", "慢跑", "跑步"]),
        ("cycling_growth", ["cycling", "腳踏車", "自行車", "河濱"]),
        ("badminton_growth", ["badminton", "羽球"]),
        ("hiking_growth", ["hiking", "健行", "郊山"]),
        ("boxing_fitness_growth", ["boxing", "拳擊有氧"]),
        ("barre_growth", ["barre", "芭蕾雕塑"]),
...
async def handle_xiaoxia_autonomy_command(message, user_input):
    ...
    try:
        ...
        await status.delete()
        result_view = PhotoResultView(context)
        sent = await _send_photo_message(message.channel, context, view=result_view, title_prefix="🌱 小俠今日自主")
        context["message_id"] = sent.id
        photo_generation_contexts[sent.id] = context
        result_view.context = context
        return True
    except Exception as exc:
...
VIBE_RENDER_GUIDE = {
    ...
    "魅": {
        ...
        "translator": "maximum alluring sensuality, chest-emphasis first, then ensure a clearly readable secondary attraction point: legline or waist-abdomen curve, or both when natural, deep V or low neckline, sweetheart neckline, off-shoulder or halter, visible collarbones and shoulder-neck line, natural cleavage, fitted upper silhouette, clear bust-waist ratio, seductive yet tasteful, unmistakable romantic tension"
    },
}
...
def _strengthen_cosplay_state_for_vibe(planned, vibe_request=None):
    ...
    if zh == "甜":
        ...
        planned["costume_direction"] = (str(planned.get("costume_direction") or "") + " This is the max-allure lane. Always prioritize chest allure first through the strongest tasteful chest-focused garment structure that fits the role, such as a deep V or low neckline, sweetheart line, off-shoulder or halter cut, visible collarbones or shoulder-neck line, natural cleavage or bust contour, fitted upper silhouette, and a clear bust-waist ratio. Then ensure at least one secondary attraction point is clearly readable — legline or waist-abdomen curve, or both when natural for the role, garment structure, scene mood, and pose logic. Legline alone does not satisfy this mode if chest allure is weak or absent. Do not soften this into merely elegant, cute, or conservative styling.").strip()
...
{{
  ...
  "canonical_visual_anchors": ["3到6個原型視覺錨點，例如主色系/服裝剪影/徽記/道具/髮型/環境"],
...
"""
    try:
        ...
        if isinstance(data, dict):
            ...
            if isinstance(data.get("canonical_visual_anchors"), str):
                data["canonical_visual_anchors"] = [x.strip() for x in re.split(r"[,，、/|]", data["canonical_visual_anchors"]) if x.strip()]
            if not isinstance(data.get("canonical_visual_anchors"), list):
                data["canonical_visual_anchors"] = []
            return data
    except Exception as exc:
        ...
        for sub_attempt in range(2):
            ...
            if candidate and not too_similar:
                ...
                event = (
                    ...
                    f"原型視覺錨點：{', '.join([str(x) for x in (candidate.get('canonical_visual_anchors') or [])]) or '保留角色主色系、服裝語彙與標誌性道具'}\n"
...
6. 若角色來自文學、古典、歷史、動漫、電影、電玩或科幻作品，需尊重該角色的時代氣質、作品世界觀、服裝語彙與標誌性視覺錨點，避免錯置成 generic 古風、generic 網美棚拍或任意黑色性感裝。
6a. L0 可以把服裝改成更成熟、更貼身、更有胸口魅力的小俠版，但必須「性感化原角色語言」，不可把角色辨識度洗掉；需保留至少 2 到 3 個原型可辨識元素，例如主色系、剪影、徽記、道具、髮型/髮色、環境或角色任務。
...
- Preserve the original role's signature visual anchors. The sexy L0 reinterpretation must be built from the role's own color palette, silhouette, emblem/motif, prop, hair/hair color, environment, or task. Do not replace the character with unrelated black lingerie, demon wings, generic fantasy, or generic glamour styling unless those are truly part of the role.
...
def _is_pose_critical_request(text):
    raw = str(text or "").lower()
    keywords = ["趴", "俯臥", "俯卧", "躺", "側躺", "侧躺", "跪", "盤腿", "盘腿", "face-down", "face down", "prone", "lying on her stomach", "lying down", "on her stomach"]
    return any(k.lower() in raw for k in keywords)


def _extract_pose_user_request(initial_prompt, visual_dict=None):
    if isinstance(visual_dict, dict):
        ...
        for key in ("user_scene_hardlock", "user_priority_request", "primary_action", "activity"):
            val = str(state.get(key) or "").strip()
            if val:
                return val
    text = str(initial_prompt or "")
    m = re.search(r"Highest-priority Daxia request to obey literally:\s*(.+?)(?:\n|$)", text, flags=re.I)
    if m:
...
# ==========================================
# 👗 v1.5.11：Figure 10 Styling Board accessory parsing
# ==========================================
STYLING_BOARD_MAJOR_ACCESSORY_RULES = [
    ("visible bag / handbag / purse from request or Figure 10 styling board", ["包包", "皮包", "手提包", "肩背包", "斜背包", "背包", "托特包", "小包", "bag", "handbag", "purse", "shoulder bag", "crossbody", "tote"]),
    ("visible necklace from request or Figure 10 styling board", ["項鍊", "項链", "頸鍊", "吊墜", "墜飾", "necklace", "pendant", "choker"]),
    ("visible earrings from request or Figure 10 styling board", ["耳環", "耳环", "耳飾", "耳饰", "earrings", "earring"]),
    ("visible bracelet / wrist accessory from request or Figure 10 styling board", ["手鍊", "手链", "手環", "手环", "bracelet", "bangle", "wrist accessory"]),
    ("visible scarf / neck scarf from request or Figure 10 styling board", ["領巾", "丝巾", "絲巾", "圍巾", "围巾", "scarf", "neck scarf"]),
    ("visible sunglasses or glasses from request or Figure 10 styling board", ["太陽眼鏡", "太阳眼镜", "墨鏡", "墨镜", "眼鏡", "眼镜", "sunglasses", "glasses"]),
    ("visible hat / hair accessory from request or Figure 10 styling board", ["帽子", "草帽", "貝雷帽", "髮飾", "发饰", "髮夾", "发夹", "hat", "cap", "beret", "hair accessory", "hair clip"]),
    ("visible belt / waist accessory from request or Figure 10 styling board", ["腰帶", "腰带", "belt"]),
    ("visible shoes from request or Figure 10 styling board", ["鞋子", "高跟鞋", "靴子", "涼鞋", "拖鞋", "sneakers", "heels", "boots", "sandals", "shoes"]),
]

def _extract_requested_styling_accessory_requirements(text):
    raw = str(text or "")
    low = raw.lower()
    out = []
    for label, keys in STYLING_BOARD_MAJOR_ACCESSORY_RULES:
...
def _extract_visual_must_have(text, mode="photo", visual_dict=None, trace_context=None):
    ...
    add("requested prop/accessory", "雨傘", "手套", "項鍊", "項链", "耳環", "耳环", "手鍊", "手链", "領巾", "絲巾", "圍巾", "太陽眼鏡", "墨鏡", "帽", "鞋", "包包", "皮包", "手提包", "肩背包", "斜背包", "劍", "法杖", "麥克風", "抱枕", "umbrella", "gloves", "hat", "sword", "staff", "microphone", "pillow", "bag", "handbag", "purse", "necklace", "earrings", "bracelet", "scarf", "sunglasses", "glasses", "shoes")
    items.extend(_extract_requested_styling_accessory_requirements(raw))
    if str(mode or "").lower() == "photo_reference":
        # v1.5.11: When a Figure 10 styling board is supplied, text-mentioned accessories are hard visible requirements.
        items.append("Figure 10 styling board: match the main clothing first, then preserve any text-mentioned major accessory visibly")
    if str(mode or "").lower() == "cosplay":
        ...
        if isinstance(trace_context, dict):
            ...
            if vibe_zh == "魅" or trace_context.get("default_allure_hard_lock") or trace_context.get("user_requested_hard_sexy"):
                ...
                items.append("L0 default allure: visible second attraction point as legline or waist-abdomen curve, or both when natural")
        if anchors:
    ...
    if str(mode or "").lower() == "cosplay":
        ...
        if isinstance(trace_context, dict):
            ...
            if vibe_zh == "魅" or trace_context.get("default_allure_hard_lock") or trace_context.get("user_requested_hard_sexy"):
                ...
                items.append("legline-only styling with weak or absent chest allure in default L0 allure cosplay")
    return _dedupe_keep_order(items)[:14]
...
def _build_pose_critical_seedream_prompt(user_request, has_reference=False, current_outfit=None, retry_reason="", visual_checklist=None):
    ...
    if current_outfit:
        lines += ["", f"Current outfit continuity, clothing only: {str(current_outfit).strip()}. It must not override pose or scene."]
    if retry_reason:
        lines += ["", "PREVIOUS OUTPUT WAS REJECTED:", retry_reason, "Correct the failure now. The required pose/action must be obeyed before beauty, fashion display, or flattering framing."]
    lines += ["", "TEXT REQUEST — this controls the final scene, pose, action, and composition:", "", user_request, "", "Mandatory obedience rules:", "- The requested body pose and scene must be visually clear.", "- Do not replace a lying/prone/face-down request with sitting, standing, kneeling, leaning, side-lying, reading, or posing beside the furniture.", "- If the request says chest/abdomen touching bed/sofa, the torso must be horizontal and supported by that surface.", "- If the request says pillow, show the pillow action clearly.", "- Natural anatomy, single person only, candid lifestyle photo."]
    return "\n".join(lines).strip()
...
Return JSON only:
{{"scene_ok":true,"pose_ok":true,"outfit_ok":true,"character_anchor_ok":true,"prop_ok":true,"chest_allure_ok":true,"second_attraction_ok":true,"sexy_ok":true,"must_have_ok":true,"must_not_have_ok":true,"overall_ok":true,"failure_reason":"","violations":[],"must_have_failures":[],"must_not_have_violations":[]}}
...
- Fail outfit_ok if the outfit becomes a generic glamour dress, generic white dress, generic black outfit, ordinary lingerie/fashion, unrelated costume, or a plain conservative working uniform instead of a sexy adaptation of the requested character costume.
- For profession, inn, maid, service, office, or other uniform-coded roles, fail outfit_ok and sexy_ok when the image keeps only a plain conservative uniform rather than an adult-feminine sexy reinterpretation.
...
- Fail prop_ok if a named prop or styling accessory such as microphone, datapad, staff, sword, tambourine, book, bag, necklace, scarf, sunglasses, hat, bracelet, earrings, or shoes is required but absent or replaced by an unrelated item.
- When the prompt or visual_checklist mentions a Figure 10 styling board accessory, the final image must visibly include that accessory. Missing major accessories should set prop_ok=false or must_have_ok=false.
- Check every visual_checklist.must_have item. Set must_have_ok=false and list failures when a concrete required item is missing (for example sleeveless rendered as short sleeves, or a requested bag/necklace/scarf/sunglasses omitted).
...
{prompt_text[:6000]}
Rules:
- Be strict about major pose/action words such as prone, face-down, lying on her stomach, lying down, sitting, standing, kneeling, hugging a pillow, holding a prop, cooking, cycling, etc.
- If the request says prone / face-down / lying on her stomach, fail pose_ok if she is sitting, standing, kneeling, side-lying, leaning upright, reading upright, or merely posed near the bed/sofa.
- If the request says sofa or bed, fail scene_ok if the furniture/location is visibly different.
- If a Figure 10 wardrobe/styling reference is requested, judge whether the main clothing reasonably matches; ignore tiny tailoring differences.
- Figure 10 may be a complete styling board containing clothing plus accessories. If the user text or visual_checklist mentions a bag, handbag, necklace, scarf, sunglasses, hat, bracelet, earrings, shoes, or other major accessory, that accessory must be visibly included; otherwise set must_have_ok=false and explain the omission.
- If there are no explicit outfit or accessory constraints, outfit_ok should be true.
...
"""
    try:
        ...
        if str(mode or "").lower() == "cosplay":
            ...
            chest_allure_ok = bool(result.get("chest_allure_ok", sexy_ok))
            second_attraction_ok = bool(result.get("second_attraction_ok", sexy_ok))
            overall = bool(result.get("overall_ok", scene_ok and pose_ok and outfit_ok and character_ok and prop_ok and chest_allure_ok and second_attraction_ok and sexy_ok and must_have_ok and must_not_have_ok))
            ok = bool(overall and scene_ok and pose_ok and outfit_ok and character_ok and prop_ok and chest_allure_ok and second_attraction_ok and sexy_ok and must_have_ok and must_not_have_ok)
            if isinstance(trace_context, dict):
                trace_context["chest_allure_ok"] = chest_allure_ok
                trace_context["second_attraction_ok"] = second_attraction_ok
        else:
    ...
    for level in range(5):
        ...
        if str(mode or "").lower() in {"photo_scene", "photo_reference", "diary", "cosplay"}:
            ...
            adherence_ok, adherence_reason, adherence_detail = await _vision_check_instruction_adherence_image_url(
                generated_image_url, current_prompt, mode=mode, trace_context=trace_context, retry_used=(level > 0)
            )
            ...
            if not adherence_ok:
                ...
                if isinstance(visual_dict, dict):
                    visual_dict["__adherence_gate_violation"] = adherence_reason
                    visual_dict["composition"] = str(visual_dict.get("composition", "")) + "\n*(Instruction adherence rejected previous output; retry with stricter scene/pose control.)*"
                if level < 1:
                    if msg:
                        await msg.edit(content=f"⚠️ 小俠發現這張沒有照大俠指定的場景/姿勢，正在重拍（原因：{adherence_reason[:120]}）...")
                    continue
...
def _normalize_command_date(value):
    """Normalize user date inputs like 2026-7-20 / 2026.07.20 / 2026/7/20 to YYYY-MM-DD."""
    raw = str(value or "").strip()
    if not raw:
        return ""
    # Keep only a date-like token if the whole command content was accidentally passed.
...
async def cosplay(ctx, *, mode: str = "auto"):
    ...
    if not vibe_mode:
        vibe_mode = {"zh": "魅", "en": "alluring-max", "level": 6}
    story_mode = mode
    if vibe_mode:
        story_mode = mode
    ...
    try:
        ...
        trace_context = {
            ...
            "user_input": _ctx_message_content(ctx, f"[auto_cosplay_task] /cosplay {mode}"),
...
def _seedream_cosplay_prompt(custom_prompt):
    return (
        ...
        "The result should be a highly polished cinematic cosplay photograph with vivid character storytelling and intense visual appeal. Preserve the source character's recognizable visual language through at least 2 or 3 anchors such as color palette, costume silhouette, emblem or motif, prop, hairstyle or hair color, environment, or role task. Do not turn the role into unrelated generic black lingerie, demon wings, or arbitrary glamour styling unless that is truly part of the source role.\n\n"
...
def _repair_clean_line(value, limit=260):
    text = _clean_text_compact(str(value or ""))
    return text[:limit] if limit and len(text) > limit else text


def _repair_collect_history(context, latest_request="", limit=4):
    items = []
    existing = (context or {}).get("repair_history")
    if isinstance(existing, list):
        items.extend([_repair_clean_line(x, 220) for x in existing if str(x or "").strip()])
    prior = _repair_clean_line((context or {}).get("repair_request"), 220)
    if prior:
        items.append(prior)
    latest = _repair_clean_line(latest_request, 220)
    if latest:
        items.append(latest)
    deduped = []
    seen = set()
...
def _seedream_repair_mode_block(context):
    ...
    lines.append("- Keep repairs local whenever possible: fix the defect while preserving the scene, pose, outfit, and mood.")
    return "\n".join(lines)
...
async def generate_seedream_v45_repair(original_image_path, repair_request, enable_safety_checker=True, trace_context=None, source_context=None):
...
async def _repair_photo_context(context, repair_request, msg=None):
    repair_history = _repair_collect_history(context, repair_request, limit=4)
    trace_context = {
        "kind": "repair",
        "action": "photo_repair",
    ...
    generated_image_url = await generate_seedream_v45_repair(source_path, repair_request, enable_safety_checker=True, trace_context=trace_context, source_context=context)
    if not generated_image_url or not str(generated_image_url).startswith("http"):
...
def _build_result_embed(context, title_prefix="📸 小俠照片", attachment_filename=None):
    """依照片類型建立完整 embed；cosplay 保留角色介紹與小俠版詮釋，不顯示內部修正要求。"""
    context = context or {}
    if str(context.get("type") or context.get("source_mode") or "").lower() == "cosplay":
        ...
        embed = discord.Embed(
            title=str(title)[:256],
            description=(post_text.get("description") or context.get("event") or context.get("message") or "小俠完成了這次 cosplay。")[:4096],
            color=0xffb6c1,
        )
        if attachment_filename:
            embed.set_image(url=f"attachment://{attachment_filename}")
        ...
        fields = [
            ...
            ("📸 今日畫面", post_text.get("scene_caption") or context.get("composition")),
            ("💌 小俠給大俠", post_text.get("message_to_daxia") or context.get("message")),
        ]
...
async def handle_photo_raw_command(message, user_input):
    ...
    if not raw_prompt:
        await message.channel.send("用法：`/photo_raw <Seedream playground prompt>`。這個模式會直接用 9 張小俠底圖，不走 planner / 衣櫃 / 美感包裝。")
        return None
    status = await message.channel.send("🧪 小俠啟動 `/photo_raw`：用 playground 等價模式直接測 Seedream...")
    trace_context = {"kind":"photo","action":"photo_raw","source_mode":"photo_scene","raw_seedream_mode":"photo_raw_playground_equivalent","user_input":raw_input,"scene_seed_text":raw_prompt,"used_pending_wardrobe":False,"wardrobe_mode":"none","wardrobe_id":None,"has_attachment":False}
    ...
    try:
        ...
        _write_generation_trace("photo", trace_context)
        await status.delete()
        await _send_photo_message(message.channel, context, view=None, title_prefix="🧪 Photo Raw")
        return local_url
    except Exception as exc:
        trace_context["error"] = str(exc)
        _write_generation_trace("photo", trace_context)
        await status.edit(content=f"⚠️ `/photo_raw` 失敗：`{str(exc)[:1500]}`")
        return None
...
def _apply_photo_to_diary(context, photo_name, overwrite=False):
    ...
    source_module = str(context.get("source_module") or context.get("source_mode") or context.get("type") or "photo").strip() or "photo"
    image_role = str(context.get("image_role") or "manual_diary_pick").strip() or "manual_diary_pick"
    activity = context.get("autonomy_activity") if isinstance(context.get("autonomy_activity"), dict) else {}
    ...
    overrides[target_date] = {
        ...
        "activity_title": str(activity.get("title") or context.get("scene_text") or photo_name).strip(),
        "episode_id": str(context.get("episode_id") or "").strip(),
        "selected_by": "daxia_button",
    }
    ...
    if str(original_context.get("type") or "").lower() == "diary" or "交換日記" in str(original_context.get("topic", "")):
        ...
        if diary_date:
            replaced, html_old_url = replace_completed_diary_image(
                ...
                description=repaired_context.get("composition", "修正版交換日記照片"),
...
class PhotoRepairModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            embed = _build_result_embed(repaired_context, title_prefix="🩹 修正版預覽", attachment_filename=filename if file else None)
            try:
                await status.delete()
            except Exception:
                pass
...
async def _create_cosplay_context_for_reroll(mode="auto", msg=None, force_new_topic=True):
    ...
    if raw_mode == "auto":
        raw_mode = "每日動態選角"
    vibe_mode = _extract_vibe_mode(raw_mode)
    if not vibe_mode:
        vibe_mode = {"zh": "魅", "en": "alluring-max", "level": 6}
    story_mode = raw_mode
    for alias in VIBE_KEYWORDS.get(vibe_mode.get("zh"), []):
    ...
    story_mode = re.sub(r"\s+", " ", story_mode).strip(" ,，") or "auto"
    if msg:
        await msg.edit(content="🔄 小俠正在重新抽一個全新的 cosplay 題材、內文與畫面…")
    story = await generate_story(story_mode if force_new_topic else "auto")
    ...
    if candidate_for_history:
        candidate_for_history.update({
            ...
            "source": "reroll_full",
            "topic": story.get("topic"),
            "charm_level": str((cosplay_state or {}).get("vibe_target_zh") or vibe_mode.get("zh") or "魅"),
            "image_url": local_url,
        })
        append_cosplay_topic_history(candidate_for_history)
    try:
