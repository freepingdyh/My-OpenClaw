# -*- coding: utf-8 -*-
"""v1.13.03 — consolidated runtime + Pose Library metadata authority.
...
app = stable_base.app
MIGRATION_VERSION = "1.13.03"


def _activate_v11303():
    activated = {}
    ...
    # Library metadata must wrap the observer after the pose-generation wrapper is installed.
    ...
    print("💃 [V11303_POSE_LIBRARY_RUNTIME_ACTIVE]")
    for name, info in activated.items():
        print(f"   • {name}: {info}")
    return activated


_ACTIVATED = _activate_v11303()

if __name__ == "__main__":
    print("🚀 [LOBSTER_ENTRYPOINT] version=1.13.03 consolidated_after=1.12.06al stable_base=1.11.17.2")
    try:
